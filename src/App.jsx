...
import { saveSettings, onSettingsChanged, submitQuizScore, loadAllScores, onScoresChanged, clearAllScores, submitPortfolio, loadAllPortfolios, onPortfoliosChanged, clearAllPortfolios, submitTimingScore, loadAllTimingScores, onTimingScoresChanged, clearAllTimingScores } from "./firebase.js";

// ─── TIMELINE BRANDING ─────────────────────────────────
const C = {
  bg:"#0c1a30",card:"#132240",border:"#1e3a5a",
  accent:"#5b9bd5",accentDim:"rgba(91,155,213,0.15)",
  green:"#34d399",greenDim:"rgba(52,211,153,0.15)",
  red:"#ff2d6a",redDim:"rgba(255,45,106,0.15)",
  amber:"#fbbf24",amberDim:"rgba(251,191,36,0.15)",
  purple:"#a78bfa",purpleDim:"rgba(167,139,250,0.15)",
  text:"#f1f5f9",textMuted:"#7ba3cc",textDim:"#4a7aa8",
...
const DEFAULT_SETTINGS = {
  ...
  passiveFee:0.15,factorFee:0.30,activeFee:0.60,
  pizzaCost:12.00,defaultGrowthRate:7.5,riskFreeRate:2.0,
  showQuiz:true,showJargon:true,showPortfolio:true,showFees:true,showPizza:true,showPick:true,showDoNothing:true,
};
const DEFAULT_QUIZ = [
  {q:"What does a passive index fund try to do?",options:["Beat the market by picking winners","Match the return of a market index","Only invest in tech stocks","Avoid all risk entirely"],correct:1,explanation:"A passive fund tracks an index like the FTSE 100 — it mirrors the market, not outsmart it."},
  {q:"Which of these is a common 'factor' in factor investing?",options:["Luck","Value","Astrology","Gut feeling"],correct:1,explanation:"Value targets stocks cheap relative to fundamentals. Others: momentum, quality, size."},
  {q:"Over 20 years, roughly what % of active managers fail to beat a simple index fund?",options:["About 25%","About 50%","About 75%","Over 90%"],correct:3,explanation:"Over 90% underperform their benchmark over 20 years, largely due to higher fees."},
  {q:"Factor investing sits somewhere between…",options:["Stocks and bonds","Passive indexing and active stock-picking","Savings accounts and property","Crypto and commodities"],correct:1,explanation:"Rules-based like passive, but tilts towards characteristics like value or momentum."},
  {q:"Why do fees matter so much in investing?",options:["They don't — performance is all that matters","They compound over time, eating into returns","Higher fees always mean better returns","Fees only apply when you sell"],correct:1,explanation:"Fees compound like returns. Even 1% difference costs tens of thousands over 30 years."},
];
const DEFAULT_JARGON = [
  {term:"Alpha",plain:"Extra return above the market. Market returns 8%, your fund returns 10% = alpha of 2%.",category:"Performance"},
  {term:"Beta",plain:"How much your investment moves with the market. Beta 1 = exactly with market.",category:"Risk"},
  {term:"Expense Ratio",plain:"Annual fee to own a fund. 0.15% is cheap (passive), 0.60% is expensive (active).",category:"Fees"},
  ...
  {term:"Dividend Yield",plain:"Annual income as % of price. £100 stock paying £3/year = 3% yield.",category:"Income"},
  {term:"Market Cap",plain:"Total value of a company's shares. Large cap = big, small cap = smaller.",category:"Size"},
...
// ─── MSCI ACWI ANNUAL RETURNS (%) ──────────────────────
const MSCI_ACWI_RETURNS = {2000:-14.2,2001:-16.2,2002:-19.3,2003:34.6,2004:15.8,2005:11.4,2006:21.5,2007:12.2,2008:-41.8,2009:35.4,2010:13.2,2011:-6.9,2012:16.8,2013:23.4,2014:4.7,2015:-1.8,2016:8.5,2017:24.6,2018:-8.9,2019:27.3,2020:16.8,2021:19.0,2022:-18.0,2023:22.8,2024:18.0};

// ─── COVID DATA (from March 2020) ─────────────────────
function genCovidData() {
  ...
  const hl={0:"🔴 Markets in sharp decline — pandemic spreading",2:"🔴 Countries entering lockdown worldwide",3:"🔴 MSCI ACWI down 34% from peak",5:"🔴 Markets hit bottom — March 23, 2020",6:"📰 Central banks announce massive stimulus",8:"📰 Tentative recovery begins",13:"📰 Economies reopening — cautious optimism",21:"📰 Approaching pre-crash levels",25:"📰 Second wave fears — wobble",33:"✅ Vaccine breakthroughs — surge",37:"✅ All-time highs — full recovery",45:"✅ Strong rally into 2021",55:"✅ Portfolio well above starting value"};
  return{weeks:raw.map((v,i)=>({week:i,value:Math.round(v*1000),headline:hl[i]||null})),sellPrompts:[2,4,8],buyPrompts:[10,18,25,33,45],startDate:new Date(2020,2,2)};
}
const covidData=genCovidData();

// ─── GROWTH GENERATOR (bias-corrected, calibrated vol) ─
function genGrowth(seed,annualVol,annualReturn,years=20){
  ...
  for(let y=0;y<=years*12;y++){
    v*=(1+(rng(seed+y*7.3)-0.5)*vol+md); // FIXED: -0.5 not -0.48
    if(y%3===0)pts.push({month:y,value:Math.round(v*100)/100});
  }
  return pts;
}
// Volatility helper (quarterly data → sqrt(4))
function calcVol(data){
  const r=[];for(let i=1;i<data.length;i++)r.push((data[i].value-data[i-1].value)/data[i-1].value);
  if(r.length<2)return 0;
  const avg=r.reduce((a,b)=>a+b,0)/r.length;
  return Math.sqrt(r.reduce((a,x)=>a+Math.pow(x-avg,2),0)/(r.length-1))*Math.sqrt(4)*100; // FIXED: sqrt(4) for quarterly
}
...
const fundPairs=[
  ...
  {aType:"Momentum Factor",bType:"Passive Index",aSeed:303,bSeed:404,insight:"Momentum delivered more but with wilder swings. Passive was more stable."},
...
function BigChart({datasets,width=700,height=300,yPrefix="£"}){
  ...
  const xLabels=datasets[0]?.xLabels||["0y","5y","10y","15y","20y"];
  return(<svg width="100%" viewBox={`0 0 ${width} ${height}`} style={{display:"block"}}>
    {yTicks.map((v,i)=>{const y=pad.top+h-((v-min)/(max-min))*h;return(<g key={i}><line x1={pad.left} x2={width-pad.right} y1={y} y2={y} stroke={C.border}/><text x={pad.left-8} y={y+4} textAnchor="end" fill={C.textDim} fontSize="11" fontFamily={F.mono}>{yPrefix}{v>=10000?`${(v/1000).toFixed(0)}k`:v>=1000?`${(v/1000).toFixed(1)}k`:Math.round(v)}</text></g>);})}
    {xLabels.map((l,i)=>l?<text key={i} x={pad.left+(w*i)/(xLabels.length-1)} y={height-6} textAnchor="middle" fill={C.textDim} fontSize="11" fontFamily={F.mono}>{l}</text>:null)}
...
function CrashChart({data,soldWeek,boughtWeek,width=700,height=300}){
  ...
  const yTicks=Array.from({length:5},(_,i)=>min+((max-min)*i)/4);
  const dateLabels=[];const sd=covidData.startDate;const step=Math.max(1,Math.floor(data.length/8));
  for(let i=0;i<data.length;i+=step){const d=new Date(sd.getTime()+i*7*86400000);dateLabels.push({idx:i,label:d.toLocaleDateString("en-GB",{month:"short",year:"2-digit"})});}
  ...
  return(<svg width="100%" viewBox={`0 0 ${width} ${height}`} style={{display:"block"}}>
    ...
    {soldWeek!=null&&boughtWeek!=null&&<rect x={x(soldWeek)} y={pad.top} width={x(boughtWeek)-x(soldWeek)} height={h} fill={C.textDim} opacity="0.08" rx="4"/>}
...
const btn={padding:"14px 28px",background:C.accent,border:"none",borderRadius:"12px",color:"#0c1a30",fontFamily:F.body,fontSize:"15px",fontWeight:700,cursor:"pointer"};
...
function Leaderboard({title,entries,columns,onBack,onRefresh,onClear}){
  ...
  const [sortCol,setSortCol]=useState(0);const [sortDir,setSortDir]=useState("desc");
  const sorted=[...entries].sort((a,b)=>{const col=columns[sortCol];const av=col.val(a),bv=col.val(b);return sortDir==="desc"?bv-av:av-bv;});
...
// ═══════════════════════════════════════════════════════
// SETTINGS (presenter only — synced to Firebase)
// ═══════════════════════════════════════════════════════
function SettingsPanel({settings,setSettings,quizQs,setQuizQs,jargonTs,setJargonTs}){
  const [editTab,setEditTab]=useState("general");
  const numFields=[{section:"Annual Returns (%)",items:[{key:"marketReturn",label:"Market (passive)"},{key:"valueReturn",label:"Value"},{key:"momentumReturn",label:"Momentum"},{key:"qualityReturn",label:"Quality"},{key:"smallCapReturn",label:"Small Cap"}]},{section:"Annualised Volatility (%)",items:[{key:"marketVol",label:"Market"},{key:"valueVol",label:"Value"},{key:"momentumVol",label:"Momentum"},{key:"qualityVol",label:"Quality"},{key:"smallCapVol",label:"Small Cap"}]},{section:"Fees (OCF %)",items:[{key:"passiveFee",label:"Passive"},{key:"factorFee",label:"Factor"},{key:"activeFee",label:"Active"}]},{section:"Other",items:[{key:"pizzaCost",label:"Pizza cost (£)"},{key:"defaultGrowthRate",label:"Growth rate (%)"},{key:"riskFreeRate",label:"Risk-free rate (%)"}]}];
  ...
  const handleNum=(key,val)=>{const n=parseFloat(val);if(!isNaN(n)){const updated={...settings,[key]:n};setSettings(updated);try{saveSettings(updated);}catch(e){}}};
  const toggleSection=(key)=>{const updated={...settings,[key]:!settings[key]};setSettings(updated);try{saveSettings(updated);}catch(e){}};
  ...
  const addQ=()=>setQuizQs([...quizQs,{q:"New question?",options:["A","B","C","D"],correct:0,explanation:"Explanation."}]);
  ...
  const addJ=()=>setJargonTs([...jargonTs,{term:"New",plain:"Definition.",category:"General"}]);
  const removeJ=(i)=>{if(jargonTs.length>1)setJargonTs(jargonTs.filter((_,j)=>j!==i));};

  return(<div>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"20px"}}><h3 style={{fontFamily:F.display,color:C.text,fontSize:"24px",margin:0}}>Settings</h3><button onClick={()=>{const d={...DEFAULT_SETTINGS};setSettings(d);setQuizQs([...DEFAULT_QUIZ]);setJargonTs([...DEFAULT_JARGON]);try{saveSettings(d);}catch(e){}}} style={{...btn,background:C.card,color:C.textMuted,border:`1px solid ${C.border}`,fontSize:"12px",padding:"8px 14px"}}>Reset defaults</button></div>
    <div style={{background:C.greenDim,border:`1px solid ${C.green}`,borderRadius:"10px",padding:"12px 16px",marginBottom:"24px"}}><span style={{color:C.green,fontFamily:F.mono,fontSize:"13px"}}>✓ Changes sync to all participants in real-time via Firebase.</span></div>
    ...
    {editTab==="quiz"&&(<div><p style={{color:C.textMuted,fontSize:"13px",marginBottom:"16px"}}>Edit quiz questions. Click letter to set correct answer.</p>{quizQs.map((q,qi)=>(<div key={qi} style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:"12px",padding:"16px",marginBottom:"12px"}}><div style={{display:"flex",justifyContent:"space-between",marginBottom:"10px"}}><span style={{fontFamily:F.mono,fontSize:"12px",color:C.textDim}}>Q{qi+1}</span><button onClick={()=>removeQ(qi)} style={{background:"none",border:"none",color:C.red,cursor:"pointer",fontFamily:F.mono,fontSize:"12px"}}>✕ Remove</button></div>
      <input value={q.q} onChange={e=>updateQ(qi,"q",e.target.value)} style={{...inputStyle,marginBottom:"8px",fontWeight:600}}/>
      ...
      <input value={q.explanation} onChange={e=>updateQ(qi,"explanation",e.target.value)} style={{...inputStyle,marginTop:"8px"}} placeholder="Explanation"/>
    </div>))}<button onClick={addQ} style={{...btn,background:C.card,color:C.accent,border:`1px solid ${C.accent}`,width:"100%"}}>+ Add question</button></div>)}
    {editTab==="jargon"&&(<div><p style={{color:C.textMuted,fontSize:"13px",marginBottom:"16px"}}>Edit jargon terms.</p>{jargonTs.map((t,ti)=>(<div key={ti} style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:"12px",padding:"16px",marginBottom:"12px"}}><div style={{display:"flex",justifyContent:"space-between",marginBottom:"10px"}}><span style={{fontFamily:F.mono,fontSize:"12px",color:C.textDim}}>Term {ti+1}</span><button onClick={()=>removeJ(ti)} style={{background:"none",border:"none",color:C.red,cursor:"pointer",fontFamily:F.mono,fontSize:"12px"}}>✕</button></div>
      <div style={{display:"flex",gap:"8px",marginBottom:"8px"}}><input value={t.term} onChange={e=>updateJ(ti,"term",e.target.value)} style={{...inputStyle,fontWeight:600}}/><input value={t.category} onChange={e=>updateJ(ti,"category",e.target.value)} style={{...inputStyle,width:"120px"}}/></div>
      <textarea value={t.plain} onChange={e=>updateJ(ti,"plain",e.target.value)} rows={2} style={{...inputStyle,resize:"vertical"}}/>
    </div>))}<button onClick={addJ} style={{...btn,background:C.card,color:C.accent,border:`1px solid ${C.accent}`,width:"100%"}}>+ Add term</button></div>)}
  </div>);
}
...
function QuizPage({userName,isPresenter,quizQs}){
  ...
  const [submitted,setSubmitted]=useState(false);const [submitting,setSubmitting]=useState(false);const [submitError,setSubmitError]=useState(null);
  const [showBoard,setShowBoard]=useState(false);const [scores,setScores]=useState([]);const [loading,setLoading]=useState(false);const [pCount,setPCount]=useState(0);
  ...
  if(done){const pct=Math.round((score/quizQs.length)*100);return(<div style={{textAlign:"center",padding:"40px 20px"}}><div style={{fontSize:"64px",marginBottom:"16px"}}>{pct>=80?"🏆":pct>=50?"👍":"📚"}</div><h2 style={{fontFamily:F.display,color:C.text,fontSize:"32px",margin:"0 0 8px"}}>{score}/{quizQs.length}</h2><p style={{color:C.textMuted,fontSize:"16px",marginBottom:"32px"}}>{pct>=80?"Excellent!":pct>=50?"Good start!":"That's what this session is for!"}</p>
    {!isPresenter&&!submitted&&!submitError&&<button onClick={handleSubmit} disabled={submitting} style={{...btn,marginBottom:"12px",opacity:submitting?0.6:1}}>{submitting?"Submitting...":"Submit score"}</button>}
    {!isPresenter&&submitError&&<div style={{padding:"16px",background:C.redDim,border:`1px solid ${C.red}`,borderRadius:"12px",marginBottom:"16px"}}><span style={{color:C.red,fontSize:"14px"}}>{submitError}</span></div>}
    {!isPresenter&&submitted&&<div style={{padding:"16px",background:C.greenDim,border:`1px solid ${C.green}`,borderRadius:"12px",marginBottom:"16px",display:"inline-block"}}><span style={{color:C.green}}>✓ Submitted!</span></div>}
    {isPresenter&&(<div style={{display:"flex",flexDirection:"column",gap:"12px",alignItems:"center"}}><div style={{padding:"12px 20px",background:C.amberDim,border:`1px solid ${C.amber}`,borderRadius:"12px"}}><span style={{color:C.amber,fontFamily:F.mono,fontSize:"14px"}}>📊 {pCount} submitted</span></div><button onClick={reveal} style={{...btn,fontSize:"18px",padding:"18px 36px"}}>{loading?"Loading...":"🏆 Leaderboard"}</button></div>)}
...
function JargonPage({jargonTs}){
  ...
  if(done)return(<div style={{textAlign:"center",padding:"60px 20px"}}><div style={{fontSize:"64px",marginBottom:"16px"}}>🎓</div><h2 style={{fontFamily:F.display,color:C.text,fontSize:"28px",margin:"0 0 12px"}}>Jargon Busted!</h2><button onClick={()=>{setIdx(0);setRevealed(false);setDone(false);}} style={{...btn,marginTop:"24px"}}>Go again</button></div>);
  return(<div>
    ...
    <div style={{background:`linear-gradient(135deg, ${C.card} 0%, #0f2847 100%)`,border:`1px solid ${C.border}`,borderRadius:"16px",padding:"40px 32px",textAlign:"center",marginBottom:"24px",minHeight:"200px",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center"}}>
...
// ═══════════════════════════════════════════════════════
// PORTFOLIO (annualised return headline, leaderboard)
// ═══════════════════════════════════════════════════════
function PortfolioPage({settings:s,userName,isPresenter}){
  const fDefs={Market:{color:C.accent,desc:"Pure passive — whole market",fee:s.passiveFee,ret:s.marketReturn,vol:s.marketVol},Value:{color:C.green,desc:"Cheap, undervalued stocks",fee:s.factorFee,ret:s.valueReturn,vol:s.valueVol},Momentum:{color:C.amber,desc:"Recent strong performers",fee:s.factorFee,ret:s.momentumReturn,vol:s.momentumVol},Quality:{color:C.purple,desc:"Profitable, stable companies",fee:s.factorFee,ret:s.qualityReturn,vol:s.qualityVol},"Small Cap":{color:C.red,desc:"Smaller companies for growth",fee:s.factorFee,ret:s.smallCapReturn,vol:s.smallCapVol}};
  const gData=useMemo(()=>({Market:genGrowth(42,s.marketVol,s.marketReturn),Value:genGrowth(99,s.valueVol,s.valueReturn),Momentum:genGrowth(17,s.momentumVol,s.momentumReturn),Quality:genGrowth(73,s.qualityVol,s.qualityReturn),"Small Cap":genGrowth(55,s.smallCapVol,s.smallCapReturn)}),[s]);
  ...
  // Net-of-fees blended portfolio: apply quarterly fee deduction
  const quarterlyFeeMultiplier=Math.pow(1-fee/100,1/4); // quarterly deduction of annual fee
  const blended=gData.Market.map((_,i)=>{
    let v=0;Object.keys(alloc).forEach(f=>v+=(alloc[f]/100)*gData[f][i].value);
    // Apply cumulative fee drag: each data point is every 3 months (quarterly)
    const quarters=i; // i-th quarterly data point
    const feeDrag=Math.pow(quarterlyFeeMultiplier,quarters);
    return{month:gData.Market[i].month,value:Math.round(v*feeDrag*100)/100};
  });
  const fv=blended[blended.length-1]?.value||10000;
  const totalRetPct=((fv-10000)/10000)*100;
  const annReturn=(Math.pow(fv/10000,1/20)-1)*100;
  const annVol=show?calcVol(blended).toFixed(1):"";
  const mktVol=show?calcVol(gData.Market).toFixed(1):"";
  const sharpe=show?((annReturn-(s.riskFreeRate||2))/(parseFloat(annVol)||1)).toFixed(2):"";
  ...
  const handleSubmitP=async()=>{setSubmitting(true);try{await submitPortfolio({name:userName,alloc:{...alloc},finalValue:Math.round(fv),totalReturn:Math.round(totalRetPct*10)/10,annReturn:Math.round(annReturn*10)/10,fee:Math.round(fee*100)/100,volatility:parseFloat(annVol),sharpe:parseFloat(sharpe)});setSubmitted(true);}catch(e){}setSubmitting(false);};
  const reveal=async()=>{try{const e=await loadAllPortfolios();setPortfolios(e);setPCount(e.length);}catch(e){}setShowBoard(true);};

  if(showBoard&&isPresenter)return(<Leaderboard title="Portfolio Leaderboard" entries={portfolios} columns={[{label:"Return p.a.",val:e=>e.annReturn||0,fmt:e=>`${e.annReturn||"—"}%`},{label:"Fee",val:e=>-(e.fee||0),fmt:e=>`${e.fee}%`},{label:"Vol",val:e=>-(e.volatility||0),fmt:e=>`${e.volatility}%`},{label:"Sharpe",val:e=>e.sharpe||0,fmt:e=>(e.sharpe||0).toFixed?e.sharpe.toFixed(2):"—"}]} onBack={()=>setShowBoard(false)} onRefresh={reveal} onClear={async()=>{await clearAllPortfolios();setPortfolios([]);setPCount(0);setShowBoard(false);}}/>);

  return(<div>
    <p style={{color:C.textMuted,fontSize:"14px",marginBottom:"28px",lineHeight:1.5}}>Build your portfolio from £10,000. Allocations should total 100%. Returns shown net of fees.</p>
    ...
    <div style={{display:"flex",justifyContent:"space-between",marginBottom:"20px"}}><span style={{fontFamily:F.mono,fontSize:"13px",color:total===100?C.green:C.red}}>Total: {total}%{total!==100?" (need 100%)":" ✓"}</span><span style={{fontFamily:F.mono,fontSize:"13px",color:C.textDim}}>Fee: {fee.toFixed(2)}%</span></div>
    ...
    {show&&total===100&&(<div style={{marginTop:"28px"}}>
      <div style={{display:"flex",gap:"16px",marginBottom:"20px",flexWrap:"wrap"}}><Stat label="Final value" value={`£${Math.round(fv).toLocaleString()}`} sub="from £10,000" color={C.accent}/><Stat label="Return (net)" value={`${annReturn.toFixed(1)}% p.a.`} sub={`${totalRetPct.toFixed(0)}% total`} color={C.green}/><Stat label="Fee" value={`${fee.toFixed(2)}%`} sub="p.a. deducted" color={C.amber}/><Stat label="Volatility" value={`${annVol}%`} sub={`vs ${mktVol}% passive`} color={C.purple}/></div>
      <div style={{background:C.card,borderRadius:"12px",border:`1px solid ${C.border}`,padding:"20px"}}><BigChart datasets={[{data:blended,color:C.accent},{data:gData.Market,color:"#ffffff",opacity:0.35}]}/><div style={{display:"flex",gap:"20px",marginTop:"12px",justifyContent:"center"}}><Legend color={C.accent} label="Your portfolio"/><Legend color="#ffffff" label="Pure passive"/></div></div>
      <div style={{marginTop:"20px",display:"flex",gap:"12px",flexWrap:"wrap",alignItems:"center"}}>
        {!isPresenter&&!submitted&&<button onClick={handleSubmitP} disabled={submitting} style={{...btn,opacity:submitting?0.6:1}}>{submitting?"Submitting...":"Submit to leaderboard"}</button>}
        {!isPresenter&&submitted&&<span style={{color:C.green,fontFamily:F.mono,fontSize:"14px",padding:"14px"}}>✓ Submitted!</span>}
...
// ═══════════════════════════════════════════════════════
// FEE EXPLORER (real MSCI ACWI + No Fee line)
// ═══════════════════════════════════════════════════════
function FeeCalcPage({settings:s}){
  const years=Object.keys(MSCI_ACWI_RETURNS).map(Number).sort();
  const availableYears=years.filter(y=>y>=2000);
  const xLabelYears=availableYears.filter(y=>y%5===0);
  // Built-in lines always reflect Settings; custom lines are independent
  const builtInLines=[{label:"Passive",rate:s.passiveFee,color:C.accent,builtIn:true},{label:"Active",rate:s.activeFee,color:C.red,builtIn:true}];
  const [customLines,setCustomLines]=useState([]);
  const [newLabel,setNewLabel]=useState("");const [newRate,setNewRate]=useState("");
  const feeLines=[...builtInLines,...customLines];
  const addFee=()=>{if(newLabel&&newRate){const colors=[C.amber,C.purple,C.green,"#f472b6","#fb923c"];setCustomLines([...customLines,{label:newLabel,rate:parseFloat(newRate),color:colors[customLines.length%colors.length],builtIn:false}]);setNewLabel("");setNewRate("");}};
  const removeFee=(i)=>{if(i>=builtInLines.length){setCustomLines(customLines.filter((_,j)=>j!==(i-builtInLines.length)));}};
  const xLabels=["2000",...availableYears.map(y=>(xLabelYears.includes(y)&&y>2000)?String(y):"")];
  // No-fee line
  let nfVal=10000;const nfData=[{value:nfVal}];
  for(const y of availableYears){nfVal=nfVal*(1+MSCI_ACWI_RETURNS[y]/100);nfData.push({value:Math.round(nfVal)});}
  // Fee lines
  const datasets=feeLines.map(fl=>{let val=10000;const data=[{value:val}];for(const y of availableYears){val=val*(1+MSCI_ACWI_RETURNS[y]/100)*(1-fl.rate/100);data.push({value:Math.round(val)});}return{data,color:fl.color,label:fl.label,finalValue:Math.round(val),xLabels};});
  const allDatasets=[{data:nfData,color:C.textMuted,opacity:0.5,xLabels},...datasets];
  return(<div>
    <p style={{color:C.textMuted,fontSize:"14px",marginBottom:"20px",lineHeight:1.5}}>Real MSCI ACWI annual returns (2000–{availableYears[availableYears.length-1]}) applied to £10,000.</p>
    <div style={{marginBottom:"20px"}}><span style={{fontFamily:F.mono,fontSize:"12px",color:C.textDim}}>FEE LINES</span>
      <div style={{display:"flex",flexDirection:"column",gap:"6px",marginTop:"8px"}}>{feeLines.map((fl,i)=>(<div key={i} style={{display:"flex",alignItems:"center",gap:"8px",padding:"8px 12px",background:C.card,borderRadius:"8px",border:`1px solid ${C.border}`}}><div style={{width:"12px",height:"12px",borderRadius:"3px",background:fl.color,flexShrink:0}}/><span style={{flex:1,fontFamily:F.body,fontSize:"13px",color:C.text}}>{fl.label}{fl.builtIn&&<span style={{fontFamily:F.mono,fontSize:"10px",color:C.textDim,marginLeft:"6px"}}>(from settings)</span>}</span><span style={{fontFamily:F.mono,fontSize:"13px",color:C.textMuted}}>{fl.rate}%</span><span style={{fontFamily:F.mono,fontSize:"13px",color:fl.color,fontWeight:700}}>→ £{datasets[i]?.finalValue?.toLocaleString()}</span>{!fl.builtIn&&<button onClick={()=>removeFee(i)} style={{background:"none",border:"none",color:C.red,cursor:"pointer",fontSize:"14px"}}>✕</button>}</div>))}
        <div style={{display:"flex",gap:"8px",alignItems:"center"}}><input value={newLabel} onChange={e=>setNewLabel(e.target.value)} placeholder="Label" style={{...inputStyle,width:"120px"}}/><input value={newRate} onChange={e=>setNewRate(e.target.value)} placeholder="%" type="number" step="0.01" style={{...inputStyle,width:"80px"}}/><button onClick={addFee} style={{...btn,fontSize:"12px",padding:"8px 14px"}}>+ Add</button></div>
      </div>
    </div>
    <div style={{background:C.card,borderRadius:"12px",border:`1px solid ${C.border}`,padding:"20px",marginBottom:"20px"}}><BigChart datasets={allDatasets} yPrefix="£"/><div style={{display:"flex",gap:"16px",marginTop:"12px",justifyContent:"center",flexWrap:"wrap"}}><Legend color={C.textMuted} label={`No fees → £${Math.round(nfVal).toLocaleString()}`}/>{feeLines.map((fl,i)=><Legend key={i} color={fl.color} label={`${fl.label} (${fl.rate}%) → £${datasets[i]?.finalValue?.toLocaleString()}`}/>)}</div></div>
    <div style={{background:C.amberDim,border:`1px solid ${C.amber}`,borderRadius:"12px",padding:"16px 20px"}}><p style={{color:C.text,fontSize:"14px",margin:0,lineHeight:1.5}}><strong style={{color:C.amber}}>Key takeaway:</strong> 2000–{availableYears[availableYears.length-1]}, the fee gap between {feeLines[0]?.label} ({feeLines[0]?.rate}%) and {feeLines[feeLines.length-1]?.label} ({feeLines[feeLines.length-1]?.rate}%) on £10,000 is <strong>£{Math.abs((datasets[0]?.finalValue||0)-(datasets[datasets.length-1]?.finalValue||0)).toLocaleString()}</strong>.</p></div>
  </div>);
}

// ═══════════════════════════════════════════════════════
// PIZZA (Passive + Active default)
// ═══════════════════════════════════════════════════════
...
function PickFundPage({settings:s}){
  ...
  const getP=(type)=>{if(type.includes("Passive"))return{vol:s.marketVol,ret:s.marketReturn};if(type.includes("Value"))return{vol:s.valueVol,ret:s.valueReturn};if(type.includes("Momentum"))return{vol:s.momentumVol,ret:s.momentumReturn};return{vol:s.qualityVol,ret:s.qualityReturn};};
  const aP=getP(pair.aType),bP=getP(pair.bType);
...
// ═══════════════════════════════════════════════════════
// DO NOTHING (sell + rebuy timing, leaderboard)
// ═══════════════════════════════════════════════════════
function DoNothingPage({userName,isPresenter}){
  ...
  const calcFinalValue=()=>{
    ...
    if(soldWeek===null)return endVal;
    const soldVal=weeks[soldWeek].value;
    if(boughtWeek===null)return soldVal;
    return Math.round(soldVal*(endVal/weeks[boughtWeek].value));
  };
  ...
  if(showBoard&&isPresenter)return(<Leaderboard title="Market Timing Leaderboard" entries={timingScores} columns={[{label:"Final £",val:e=>e.finalValue,fmt:e=>`£${e.finalValue?.toLocaleString()}`},{label:"Return",val:e=>e.returnPct,fmt:e=>`${e.returnPct>0?"+":""}${e.returnPct}%`}]} onBack={()=>setShowBoard(false)} onRefresh={revealT} onClear={async()=>{await clearAllTimingScores();setTimingScores([]);setTCount(0);setShowBoard(false);}}/>);

  if(phase==="intro")return(<div style={{textAlign:"center",padding:"40px 20px"}}><div style={{fontSize:"64px",marginBottom:"16px"}}>😰</div><h2 style={{fontFamily:F.display,color:C.text,fontSize:"28px",margin:"0 0 12px"}}>The "Do Nothing" Challenge</h2><p style={{color:C.textMuted,fontSize:"15px",maxWidth:"500px",margin:"0 auto 12px",lineHeight:1.6}}>It's <strong style={{color:C.accent}}>March 2020</strong>. You have <strong style={{color:C.accent}}>£100,000</strong> in the <strong style={{color:C.accent}}>MSCI ACWI</strong>. A pandemic is unfolding.</p><p style={{color:C.textMuted,fontSize:"15px",maxWidth:"500px",margin:"0 auto 12px",lineHeight:1.6}}>You'll be asked: <strong style={{color:C.amber}}>sell or hold?</strong> If you sell, decide <strong style={{color:C.green}}>when to buy back in</strong>.</p><p style={{color:C.textDim,fontSize:"13px",margin:"0 auto 32px"}}>MSCI ACWI fell 34% peak-to-trough in 5 weeks.</p><button onClick={startSim} style={{...btn,fontSize:"18px",padding:"18px 40px"}}>Start →</button></div>);

  // RESULTS
  const showResults=(rt)=>{
    const fv=calcFinalValue();const heldVal=weeks[weeks.length-1].value;
    const isGood=rt==="held-result";
    return(<div>
      <div style={{textAlign:"center",marginBottom:"24px"}}><div style={{fontSize:"48px",marginBottom:"8px"}}>{isGood?"💎":"🔄"}</div><h2 style={{fontFamily:F.display,color:C.text,fontSize:"24px",margin:"0 0 8px"}}>{isGood?"Diamond hands!":rt==="stayed-cash-result"?"You stayed out!":"You tried to time it!"}</h2></div>
      ...
      <div style={{display:"flex",gap:"12px",marginBottom:"20px",flexWrap:"wrap"}}><Stat label="Your result" value={`£${fv.toLocaleString()}`} sub={`${((fv-startVal)/startVal*100).toFixed(1)}%`} color={fv>=startVal?C.green:C.red}/><Stat label="If held" value={`£${heldVal.toLocaleString()}`} sub={`+${((heldVal-startVal)/startVal*100).toFixed(1)}%`} color={C.green}/>{fv<heldVal&&<Stat label="Cost of timing" value={`£${(heldVal-fv).toLocaleString()}`} color={C.amber}/>}</div>
      <div style={{background:isGood?C.greenDim:C.amberDim,border:`1px solid ${isGood?C.green:C.amber}`,borderRadius:"12px",padding:"16px 20px",marginBottom:"20px"}}><p style={{color:C.text,fontSize:"14px",margin:0,lineHeight:1.6}}><strong style={{color:isGood?C.green:C.amber}}>{isGood?"The lesson:":"What happened:"}</strong> {isGood?"You held through a 34% crash. Time in the market wins.":rt==="stayed-cash-result"?`You sold at ${getDateLabel(soldWeek)} and never bought back.`:`You sold at ${getDateLabel(soldWeek)} and bought back at ${getDateLabel(boughtWeek)}.`}</p></div>
      <div style={{display:"flex",gap:"12px",flexWrap:"wrap",alignItems:"center"}}>
        {!isPresenter&&!submitted&&<button onClick={handleSubmitT} disabled={submitting} style={{...btn,opacity:submitting?0.6:1}}>{submitting?"...":"Submit to leaderboard"}</button>}
        {!isPresenter&&submitted&&<span style={{color:C.green,fontFamily:F.mono,fontSize:"14px"}}>✓ Submitted!</span>}
        {isPresenter&&<><span style={{color:C.amber,fontFamily:F.mono,fontSize:"13px"}}>📊 {tCount} submitted</span><button onClick={revealT} style={{...btn,fontSize:"13px",padding:"10px 20px"}}>🏆 Leaderboard</button></>}
        <button onClick={restart} style={{...btn,background:C.card,color:C.text,border:`1px solid ${C.border}`}}>Try again</button>
      </div>
    </div>);
  };
  if(phase==="held-result"||phase==="stayed-cash-result"||phase==="rebought-result")return showResults(phase);
  if(phase==="inCash"&&weekIdx>=weeks.length-1){setPhase("stayed-cash-result");return null;}
  if(phase==="playing"&&weekIdx>=weeks.length-1&&soldWeek!==null&&boughtWeek!==null){setPhase("rebought-result");return null;}

  // PLAYING
  const inCash=phase==="inCash";
  return(<div>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"16px",flexWrap:"wrap",gap:"8px"}}><span style={{fontFamily:F.mono,fontSize:"13px",color:C.textDim}}>{getDateLabel(weekIdx)} — Week {weekIdx+1}/{weeks.length}</span><div style={{display:"flex",gap:"8px",alignItems:"center"}}>{inCash&&<span style={{fontFamily:F.mono,fontSize:"11px",color:C.amber,background:C.amberDim,padding:"4px 10px",borderRadius:"6px"}}>💰 CASH</span>}<span style={{fontFamily:F.mono,fontSize:"11px",color:C.textDim}}>SPEED:</span>{[{l:"1x",v:300},{l:"2x",v:150},{l:"4x",v:75}].map(sp=><button key={sp.l} onClick={()=>setSpeed(sp.v)} style={{...chip,padding:"4px 10px",fontSize:"11px",background:speed===sp.v?C.accentDim:C.card,borderColor:speed===sp.v?C.accent:C.border,color:speed===sp.v?C.accent:C.textMuted}}>{sp.l}</button>)}</div></div>
    <div style={{display:"flex",gap:"20px",marginBottom:"20px",flexWrap:"wrap"}}><Stat label={inCash?"Cash":"Portfolio"} value={inCash?`£${weeks[soldWeek].value.toLocaleString()}`:`£${currentVal.toLocaleString()}`} color={inCash?C.amber:(currentVal>=startVal?C.green:C.red)}/>{!inCash&&<Stat label="Change" value={`${parseFloat(change)>0?"+":""}${change}%`} color={parseFloat(change)>=0?C.green:C.red}/>}{inCash&&<Stat label="Market now" value={`£${currentVal.toLocaleString()}`} sub={currentVal>weeks[soldWeek].value?"Above your exit":"Below your exit"} color={currentVal>weeks[soldWeek].value?C.red:C.green}/>}</div>
    ...
    {isAtSellPrompt&&(<div style={{background:`linear-gradient(135deg, ${C.amberDim} 0%, rgba(251,191,36,0.05) 100%)`,border:`2px solid ${C.amber}`,borderRadius:"16px",padding:"28px",textAlign:"center"}}><div style={{fontSize:"32px",marginBottom:"8px"}}>⚠️</div><h3 style={{fontFamily:F.display,color:C.amber,fontSize:"22px",margin:"0 0 8px"}}>{getDateLabel(weekIdx)}</h3><p style={{color:C.text,fontSize:"15px",marginBottom:"20px"}}>Portfolio {parseFloat(change)<0?"down":"up"} <strong>{Math.abs(parseFloat(change))}%</strong>.</p><div style={{display:"flex",gap:"16px",justifyContent:"center"}}><button onClick={sell} style={{...btn,background:C.red,color:"white",fontSize:"16px",padding:"16px 32px"}}>😰 Sell</button><button onClick={hold} style={{...btn,background:C.green,color:"#0c1a30",fontSize:"16px",padding:"16px 32px"}}>💎 Hold</button></div></div>)}
    {isAtBuyPrompt&&(<div style={{background:`linear-gradient(135deg, ${C.greenDim} 0%, rgba(52,211,153,0.05) 100%)`,border:`2px solid ${C.green}`,borderRadius:"16px",padding:"28px",textAlign:"center"}}><div style={{fontSize:"32px",marginBottom:"8px"}}>🤔</div><h3 style={{fontFamily:F.display,color:C.green,fontSize:"22px",margin:"0 0 8px"}}>Buy back in? — {getDateLabel(weekIdx)}</h3><p style={{color:C.text,fontSize:"15px",marginBottom:"8px"}}>Sold at <strong>£{weeks[soldWeek].value.toLocaleString()}</strong>. Market now <strong>£{currentVal.toLocaleString()}</strong>.</p><p style={{color:C.textMuted,fontSize:"13px",marginBottom:"20px"}}>{currentVal>weeks[soldWeek].value?"Market above your exit — buying back locks in a loss.":"Still below — but will it drop more?"}</p><div style={{display:"flex",gap:"16px",justifyContent:"center"}}><button onClick={buyBack} style={{...btn,background:C.green,color:"#0c1a30",fontSize:"16px",padding:"16px 32px"}}>📈 Buy back</button><button onClick={stayInCash} style={{...btn,background:C.card,color:C.text,border:`1px solid ${C.border}`,fontSize:"16px",padding:"16px 32px"}}>💰 Stay in cash</button></div></div>)}
...
export default function App(){
  ...
  // Listen for settings changes from Firebase (participants get presenter's settings)
  useEffect(()=>{
    if(isPresenter)return; // Presenter controls settings, doesn't listen
    const unsub=onSettingsChanged((data)=>{
  ...
  const join=()=>{if(canJoin)setScreen("main");};
  ...
  return(
    <div style={{minHeight:"100vh",background:C.bg,color:C.text,fontFamily:F.body}}>
      <div style={{maxWidth:"960px",margin:"0 auto",padding:"32px 24px"}}>
        ...
        <div style={{background:`linear-gradient(135deg, ${C.card} 0%, #0f2847 100%)`,border:`1px solid ${C.border}`,borderRadius:"16px",padding:"32px"}}>
        ...
        <div style={{textAlign:"center",marginTop:"24px",fontFamily:F.mono,fontSize:"11px",color:C.textDim}}>Simulated data for educational purposes — not financial advice. Returns based on MSCI research (1975–2025).</div>
      </div>
    </div>
  );
}
